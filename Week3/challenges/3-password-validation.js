...
const isValid = ({ times, letter, password }) => {
  const timesArr = times.split('-').map((element) => +element);
  const passwordArr = [...password];
  const letterCount = passwordArr.filter((char) => char === letter).length;

  if (letterCount >= timesArr[0] && letterCount <= timesArr[1]) {
    return `${password} is VALID, ${letter} is present ${letterCount} times and should have been present at least ${timesArr[0]} and at most ${timesArr[1]} times`;
  } else {
    return `${password} is INVALID, ${letter} is present ${letterCount} times and should have been present at least ${timesArr[0]} and at most ${timesArr[1]} times`;
  }
};
